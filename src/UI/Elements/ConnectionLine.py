...
class ConnectionLine(QGraphicsLineItem):
    ...
    def createConnectionBetweenNodes(self, io_item_1, io_item_2):
        ...
        self._first_item.getNodeItem().moved.connect(
            lambda: self.updateLine(
                self._first_item.centerPos(), self._last_item.centerPos()
            )
        )
        self._last_item.getNodeItem().moved.connect(
            lambda: self.updateLine(
                self._first_item.centerPos(), self._last_item.centerPos()
            )
        )
